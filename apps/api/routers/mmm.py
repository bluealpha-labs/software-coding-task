"""
MMM-specific API endpoints for Meridian model integration

This router provides endpoints for:
- Channel contributions over time
- Response curves per channel  
- AI-powered explanations for charts
"""
...
@router.get("/contributions", response_model=ContributionsResponse, summary="Get channel contributions", description="Get channel contribution data from the Meridian model")
...
async def get_contributions(
    ...
    """Get channel contributions from the Meridian model"""
    start_time = time.time()
    
    # Create cache key
    cache_key_str = cache_key("mmm", "contributions", window=window, group_by=group_by, user_id=current_user.id)
    ...
    logger.info(f"Contributions request completed in {duration:.3f}s for user {current_user.email}")
    
    return ContributionsResponse(**response_data)

@router.get("/response-curves/{channel}", response_model=ResponseCurveResponse, summary="Get response curve", description="Get response curve data for a specific channel")
@limiter.limit("30/minute")
async def get_response_curve(
    ...
    # Create cache key
    cache_key_str = cache_key("mmm", "response_curve", channel=channel, user_id=current_user.id)
    ...
    # Log performance
    duration = time.time() - start_time
    logger.info(f"Response curve request for {channel} completed in {duration:.3f}s for user {current_user.email}")
    
    return ResponseCurveResponse(**response_data)

@router.post("/ai/explain", response_model=AIExplainResponse, summary="Get AI explanation", description="Get AI-powered explanation for a chart context")
...
async def explain_chart(
    ...
    """Get AI explanation for a chart context"""
    start_time = time.time()
    ...
    try:
        ...
        # Log performance
        duration = time.time() - start_time
        logger.info(f"AI explanation request completed in {duration:.3f}s for user {current_user.email}")
...
@router.get("/health", response_model=HealthResponse, summary="MMM health check", description="Check health of MMM model and services")
@limiter.limit("10/minute")
async def mmm_health_check(request: Request, current_user: User = Depends(get_current_user)):
    """Health check for MMM services"""
    health_status = {
        "status": "ok",
        "timestamp": time.time(),
        "services": {},
